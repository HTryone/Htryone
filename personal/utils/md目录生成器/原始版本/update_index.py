...
# 不想被索引的文件夹名称
IGNORE_DIRS = {".git", ".obsidian", "assets", "images", "附件", "__pycache__", "templates", "build"}
# 要扫描的文件扩展名
...
def render_folder_list(tree, base_path="", depth=0):
    ...
    for name, content in items:
        if isinstance(content, dict):
            ...
            if depth == 0:
                # 主文件夹用 H2，无缩进
                lines.append(f"## 📁 {full_path}/ （{file_count} 篇）")
                lines.append("")
            else:
                # 其他层级用缩进列表
                indent = "  " * depth
                lines.append(f"{indent}- 📁 **{full_path}/** （{file_count} 篇）")
...
def generate_md_index(base_dir):
    ...
    # 渲染文件夹列表
    folder_lines = render_folder_list(folder_tree)
